...
INDEX_PATH = 'wsj/13-34.1/wsj1/doc/indices/si_tr_s.ndx'
...
if __name__ == '__main__':
    ...
    if not os.path.exists(OUTPUT_DIR):
        os.mkdir(OUTPUT_DIR)
    wav_df = get_all_wav_files_in_train(INDEX_PATH)
    ...
    columns = final_manifest.columns
    cols_new_order = [columns[0], columns[2], columns[1]]
    final_manifest = final_manifest[cols_new_order]
    print(len(final_manifest))
    print(final_manifest.head())
    final_manifest.to_csv("wsj_train_manifest.csv", index = False)

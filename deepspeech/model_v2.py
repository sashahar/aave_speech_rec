...
class BatchRNN(nn.Module):
    def __init__(self, input_size, hidden_size, nb_layers=NB_LAYERS, rnn_type=nn.LSTM, bidirectional=False, batch_norm=True):
...
class DeepSpeech(nn.Module):
    def __init__(self, rnn_hidden_size, use_mfcc_features = False, sample_rate=audio_conf['sample_rate'], nb_layers=NB_LAYERS, window_size=audio_conf['window_size']):
        ...
        self.rnn = BatchRNN(input_size=rnn_input_size, hidden_size=rnn_hidden_size, nb_layers=self.nb_layers,
                       rnn_type=RNN_TYPE, bidirectional=True, batch_norm=False)

...
parser.add_argument(
    "--saved-output", default="", type=str, help="Path to output from test.py"
)
parser.add_argument(
    "--lm-alpha-from",
    default=0.0,
...
parser.add_argument(
    "--lm-alpha-to", default=3.0, type=float, help="Language model weight end tuning"
)
parser.add_argument(
    "--lm-beta-from",
    default=0.0,
...
parser.add_argument(
    "--lm-num-betas", default=8, type=int, help="Number of beta candidates for tuning"
)
...
parser.add_argument(
    "--num-workers", default=16, type=int, help="Number of parallel decodes to run"
)
...
def decode_dataset(params):
    ...
    for out, sizes, target_strings in saved_output:
        (
            decoded_output,
            _,
        ) = decoder.decode(torch.from_numpy(out), torch.from_numpy(sizes))
...
if __name__ == "__main__":
    ...
    params_grid = [
        (float(alpha), float(beta)) for alpha in cand_alphas for beta in cand_betas
    ]

...
class DeepSpeech(nn.Module):
    ...
    def forward(self, x, lengths, total_length):
        ...
        if self.use_mfcc_features:
            ...
            )  # Collapse feature dimension NxFxT
            ...
            x, _ = self.conv(
                x, output_lengths
            )  # X has shape: batch x 32 (num_channels) x rnn_input_size//32 x f(padded_seq_len)
            sizes = x.size()
            x = x.view(
                sizes[0], sizes[1] * sizes[2], sizes[3]
...
class SimpleLSTMClassifier(nn.Module):
    def __init__(
        self, hidden_size=128, bidirectional=False, vocab_size=29, output_size=2
    ):
    ...
    def forward(self, input, input_lengths):
        ...
        x = nn.utils.rnn.pack_padded_sequence(
            input, input_lengths
        )  # Transforms from #NxTxH to (sum(T_i)xH)

...
parser.add_argument(
    "--lm-path", help="path to ARPA-format language model", default=None
)
    ...
    for i, (data) in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        if save_output is not None:
            ...
            for j in range(len(target_strings)):
                ...
                output_text.append(
                    "{},{},{}".format(
                        curr_file, decoded_output[j][0], target_strings[j][0]
                    )
                )
...
def evaluate_adversarial(test_loader, device, model, decoder, target_decoder):
    ...
    print(
        "Generic WER confidence interval:({}, {}) mean: {}".format(
            np.mean(np.array(generic_wer_vec))
            - 1.96 * np.std(np.array(generic_wer_vec)) / np.sqrt(generic_n),
            np.mean(np.array(generic_wer_vec))
            + 1.96 * np.std(np.array(generic_wer_vec)) / np.sqrt(generic_n),
            np.mean(np.array(generic_wer_vec)),
        )
    ...
    print(
        "Generic CER confidence interval:({}, {}) mean: {}".format(
            np.mean(np.array(generic_cer_vec))
            - 1.96 * np.std(np.array(generic_cer_vec)) / np.sqrt(generic_n),
            np.mean(np.array(generic_cer_vec))
            + 1.96 * np.std(np.array(generic_cer_vec)) / np.sqrt(generic_n),
            np.mean(np.array(generic_cer_vec)),
        )
    ...
    print(
        "Accent WER confidence interval:({}, {}) mean: {}".format(
            np.mean(np.array(accent_wer_vec))
            - 1.96 * np.std(np.array(accent_wer_vec)) / np.sqrt(accent_n),
            np.mean(np.array(accent_wer_vec))
            + 1.96 * np.std(np.array(accent_wer_vec)) / np.sqrt(accent_n),
            np.mean(np.array(accent_wer_vec)),
        )
    ...
    print(
        "Accent CER confidence interval:({}, {}) mean: {}".format(
            np.mean(np.array(accent_cer_vec))
            - 1.96 * np.std(np.array(accent_cer_vec)) / np.sqrt(accent_n),
            np.mean(np.array(accent_cer_vec))
            + 1.96 * np.std(np.array(accent_cer_vec)) / np.sqrt(accent_n),
            np.mean(np.array(accent_cer_vec)),
        )
...
if __name__ == "__main__":
    ...
    if not args.beam_decode:
        ...
        decoder = BeamCTCDecoder(
            characters, lm_path=args.lm_path, alpha=args.alpha, beta=args.beta
        )

    target_decoder = GreedyDecoder(characters)

    test_dataset = AudioDataset(
        manifest_filepath=args.test_manifest, char_vocab_path=args.char_vocab_path
    )
    ...
    if args.adversarial:
        ...
        print(
            ...
            "Accent CER {accent_cer:.3f}\t".format(
                wer=wer,
                cer=cer,
                generic_wer=generic_wer,
                generic_cer=generic_cer,
                accent_wer=accent_wer,
                accent_cer=accent_cer,
            )
        )
    else:

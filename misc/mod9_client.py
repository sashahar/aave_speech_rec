...
EOF_MARKER = (
    "END-OF-FILE"  # configurable sequence that's unlikely to occur in PCM audio data
)
...
class RawDescriptionWithDefaultsHelpFormatter(
...
if __name__ == "__main__":
    ...
    parser.add_argument("host", nargs="?", default="localhost", help="Server hostname.")
    parser.add_argument(
        "port", nargs="?", type=int, default=9900, help="Server TCP port."
    )
    parser.add_argument(
        "--command", "-c", default="decode", help="Command to send to server."
    )
    parser.add_argument(
        "--format",
        "-f",
    ...
    parser.add_argument(
        "--latency", "-l", type=float, help="Desired real-time latency, in seconds."
    )
    parser.add_argument(
        "--partial",
        "-p",
    ...
    parser.add_argument(
        "--timestamp", action="store_true", help="Output word-level time intervals."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="INFO",
        help="Display log messages at and above this level.",
    )
    parser.add_argument(
        "--version",
        action="store_true",
    ...
    if args.command == "decode":
        ...
        if args.batch:
            ...
            options_json[
                "batch-threads"
            ] = 4  # multi-process on server side using 4 threads
        ...
        if args.audio_format == "raw":
            ...
            logging.error(
                "Audio sample rate should only be specified for raw audio format."
            )
            sys.exit(1)
    ...
    all_wav_files = [
        (options_json, elem) for elem in pd.read_csv(MANIFEST_FILE)["wav_path"]
    ]  # [(options_json, join(WAV_DIR, f)) for f in os.listdir(WAV_DIR) if isfile(join(WAV_DIR, f)) and f.split(".")[-1] == 'wav']
